...
export const getPreviousWorkoutEntry = async ({ workoutId, exerciseId, beforeDateKey = null, exerciseMeta = null }) => {
  const history = await getWorkoutHistory();
  const targetDateKey = isValidDateKey(beforeDateKey) ? String(beforeDateKey) : getLocalDateKey();
  const dateKeys = Object.keys(history || {})
    .filter((dateKey) => isValidDateKey(dateKey) && dateKey < targetDateKey)
    .sort((a, b) => b.localeCompare(a));

  for (const dateKey of dateKeys) {
    const exactEntry = history[dateKey]?.sessions?.[workoutId]?.exercises?.[exerciseId];
    const normalizedExact = normalizeWorkoutEntry(exactEntry, exerciseMeta || {});
    if (normalizedExact) return normalizedExact;

    const sessions = Object.values(history[dateKey]?.sessions || {});
    for (const session of sessions) {
      const fallbackEntry = session?.exercises?.[exerciseId];
      const normalizedFallback = normalizeWorkoutEntry(fallbackEntry, exerciseMeta || {});
